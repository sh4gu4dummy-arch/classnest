 ...
 * popup sign-in (or email/password) it can't rely on the session cookie — it
 * authenticates with a bearer token instead. The `onRequest` hook attaches that
 * token when present; when deployed (cookie auth) no token is stored, so nothing
 * changes.
 *
 * Email/password: Better Auth's bearer plugin returns the session token on
 * `set-auth-token` (and exposes it via CORS). We capture that into sessionStorage
 * so sign-in works even when the browser blocks third-party cookies in the
...
export const authClient = createAuthClient({
  ...
  fetchOptions: {
    ...
    onSuccess(ctx) {
      // Prefer the full cookie-shaped token from the bearer plugin header.
      const headerToken = ctx.response.headers.get("set-auth-token");
      if (headerToken) {
        setBearerToken(headerToken);
        return;
      }
      // Fallback: some email/password responses put `token` in the JSON body
      // (token without signature). Still useful if the header was stripped.
      try {
...
// preview after a popup / email sign-in, so the cookie path is untouched elsewhere.
